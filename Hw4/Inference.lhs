...
ti env (e1 `ECom` e2) = 
    do  (s1, t1) <- ti env e1
        (s2, t2) <- ti (apply s1 env) e2
        let s3   = s2 `after` s1
        return (s3, apply s2 (t1 `TCom` t2) )
ti env (EFst e)       =
    do  (s, t)   <- ti env e
        tv1      <- freshTVbl "a"
        tv2      <- freshTVbl "b"
        s2       <- mgu (t) (tv1 `TCom` tv2)
        return (s2 `after` s, apply s2 tv1)
ti env (ESnd e)       =
    do  (s, t)   <- ti env e
        tv1      <- freshTVbl "a"
        tv2      <- freshTVbl "b"
        s2       <- mgu (t) (tv1 `TCom` tv2)
        return (s2 `after` s, apply s2 tv2)
...
ti env (EIsNil e)      = 
    do  (s, t)   <- ti env e
        tv       <- freshTVbl "a"
        s2       <- mgu (t) (TList tv)
        return (s2 `after` s, TBool)
ti env (EDcons e)      =
    do  (s, t)   <- ti env e
        tv       <- freshTVbl "a"
        s2       <- mgu (t) (TList tv)
        return (s2 `after` s, TCom (apply s2 tv) (apply s2 t))
...
ti env (ERec x e1 e2) = 
    do  tv <- freshTVbl "a"
        let env'  = env \\ (x, Forall [] tv)
        (s1, tx) <- ti env' e1
        s2 <- mgu (apply s1 tv) tx
        let s3    = s2 `after` s1
            t1    = generalize (apply s3 env) (apply s3 tx)
            env'' = env \\ (x, t1)
